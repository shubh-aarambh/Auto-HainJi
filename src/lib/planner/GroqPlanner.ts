...
export class GroqPlanner implements Planner {
  async recover(
    ...
    if (!apiKey) {
      logger.warn('GROQ_API_KEY is not set in environment variables. GroqPlanner will return empty list.');
      return [];
    }

    logger.info('Querying Groq Planner (llama3-70b-8192) for recovery actions...');
    ...
    const systemPrompt = `You are the AI Planner layer of Auto-HainJi AI browser automation agent.
...
Visible DOM Elements: ${JSON.stringify(elementsSummary.slice(0, 40))}`;
    ...
    try {
      ...
      logger.info(`Groq response: ${text}`);

      const data = JSON.parse(text);
      return data.actions || [];
    } catch (error) {
      logger.error('Failed to query Groq Planner', error);
      return [];
    }
  }
}
