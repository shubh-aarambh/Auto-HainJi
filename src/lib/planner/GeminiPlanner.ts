...
export class GeminiPlanner implements Planner {
  async recover(
    ...
    if (!apiKey) {
      logger.warn('GEMINI_API_KEY is not set in environment variables. GeminiPlanner will return empty list.');
      return [];
    }

    logger.info('Querying Gemini Planner (gemini-2.5-flash) for recovery actions...');
...
Visible DOM Elements: ${JSON.stringify(elementsSummary.slice(0, 40))}`;
    ...
    try {
      ...
      logger.error('Failed to query Gemini Planner', error);
      return [];
    }
  }
}
